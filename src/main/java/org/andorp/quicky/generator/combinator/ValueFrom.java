 ...
 */
public final class ValueFrom<T> implements IGenerator<T> {
     ...
     */
    public ValueFrom(Collection<IGenerator<T>> generators) {
    ...
    public T next(RandomAndSize random) {
        final IGenerator<T> g = generators.get(choose.next(random));
        return g.next(random);
    }
